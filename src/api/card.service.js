...
export class CardService {
	 ...
	 * @param {Promise} onSuccess A Promise object that resolves to the response from the API
	 */
	 ...
	 * @returns {Promise} A promise thar resolves with the arQuery response.
	 */
	transfer({ amount, toCardNumber }, onSuccess) {
		return arQuery({
			...
			onSuccess: () => {
				this.notificationService.show(
					'success',
					'Transfer successfully comleted!'
				)
				onSuccess()
			},
		})
