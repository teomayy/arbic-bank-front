...
/**
 * Represents the AQuery class for working with DOM elements
 */
class AQuery {
	/**
	 *  Create a new AQuery instance
	 ...
	 * @returns {AQuery} a new AQuery instance for the found element
	 */
	find(selector) {
	 ...
	 * @returns {AQuery} The current AQuery instance for chaining
	 */
	append(childElement) {
	 ...
	 * @returns {AQuery|string} The current AQuery instance for the chaining when setting HTML content, or the current inner HTML when getting
	 */
	html(htmlContent) {
	...
	/**
	 * Get or Set the inner text of the selected element.
	 * @param {string} htmlContent - Optional text content to set. If not provided, the current text content will be returned.
	 * @returns {AQuery|string} The current AQuery instance for the chaining when setting text content, or the current text content when getting
	 */
	text(htmlContent) {
		if (typeof htmlContent === 'undefined') {
			return this.element.textContent
		} else {
			this.element.textContent = htmlContent
			return this
		}
	}
	...
	numberInput(limit) {
		if (
			...
			throw new Error('Element must be an input with number')
		}
	 ...
	 * @param {string} [value] - The value of the set for the attribute. If not provided, the current value of the attribute will be returned.
	 * @returns {AQuery|string} The current AQuery instance for chaining (if setting) or the attribute value (if getting)
	 */
	attr(attributeName, value) {
		if (typeof attributeName !== 'string') {
			throw new Error('Attribute name bust be a string')
		}
...
/**
 * Create a new AQuery instance for the given selector.
 * @param {string|HTMLElement} selector - A CSS selector or an HTMLElement
 * @returns {AQuery} A new AQuery instance for the given selector
 */
